...
from architectures import nn_blocks
from architectures import nn_ops
...
def build_block_specs(block_specs=None):
  ...
  if not block_specs:
    block_specs = mnasnet_defaults.MNASNET_A1_BLOCK_SPECS
  if len(block_specs) != mnasnet_defaults.MNASNET_NUM_BLOCKS:
    raise ValueError('The block_specs of MnasNet must be a length {} list.'
                     .format(mnasnet_defaults.MNASNET_NUM_BLOCKS))
      ...
      for i, block_spec in enumerate(self._block_specs):
        with tf.variable_scope('block_{}'.format(i)):
          for j in range(block_spec.num_repeats):
            strides = 1 if j > 0 else mnasnet_defaults.MNASNET_STRIDES[i]
